...
async function testPlaywrightRecorder() {
  ...
  try {
    ...
    const browserPid = browser.process?.()?.pid || 'unknown'
    console.log('✅ Browser launched successfully (PID: ' + browserPid + ')\n')
    ...
    // ═══ Test 3b: Navigate to page (to ensure context is set up) ═══
    console.log('📌 Test 3b: Navigate to URL...')
    try {
      await page.goto('about:blank', { waitUntil: 'networkidle', timeout: 5000 }).catch(() => {})
    } catch (e) {
      // Navigation errors are OK for this test
    }
